...
class Compile
{
    ...
    public function get_contents($metadata, ?callable $include_post = null): array
    {
        ...
        $block_post_types = array_filter(get_post_types(['show_in_rest' => \true]), static function ($post_type): bool {
            return post_type_supports($post_type, 'editor') && apply_filters('use_block_editor_for_post_type', \true, $post_type);
        });
        $post_types = apply_filters('f!windpress/integration/gutenberg/compile:get_contents.post_types', array_values(array_unique(array_merge(['post', 'page', 'wp_template', 'wp_template_part', 'wp_block'], $block_post_types))));
        ...
        foreach ($wpQuery->posts as $post) {
             ...
             */
            if ($include_post !== null && !$include_post($post) || apply_filters('f!windpress/integration/gutenberg/compile:get_contents.skip', \false, $post)) {
                continue;
            }
            ...
            if (apply_filters('f!windpress/integration/gutenberg/compile:get_contents.dump_parsed_block', \true, $post)) {
                ...
                $block_dump = has_filter('block_parser_class') === \false && has_filter('all') === \false ? ExtractionCache::remember('gutenberg.blocks', 'post:' . $post->ID, ['content' => $raw_content, 'wordpress_version' => $GLOBALS['wp_version'] ?? ''], $dump_blocks) : $dump_blocks();
