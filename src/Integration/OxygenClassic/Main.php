...
class Main implements IntegrationInterface
{
    ...
    {
        $providers[] = ['id' => $this->get_name(), 'name' => __('Oxygen Classic Builder', 'windpress'), 'description' => __('Oxygen Classic Builder integration', 'windpress'), 'source_index' => 1, 'callback' => Config::get(sprintf('integration.%s.compile.enabled', $this->get_name()), \true) ? \WindPress\WindPress\Integration\OxygenClassic\Compile::class : static fn() => [], 'enabled' => $this->is_enabled(), 'type' => 'plugin', 'homepage' => 'https://oxygenbuilder.com/ref/12/', 'is_installed_active' => static function () {
            $is = -1;
