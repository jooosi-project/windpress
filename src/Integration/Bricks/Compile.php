...
class Compile
{
    ...
    public function get_contents($metadata): array
    {
        ...
        if ($scan->is_first_batch()) {
            array_push($contents, ['source_id' => 'option:bricks_components', 'name' => 'bricks_components', 'content' => $this->get_components()]);
        }
    ...
    public function get_post_metas($post_id): array
    {
        ...
        foreach ($this->post_meta_keys as $post_metum_key) {
            ...
            if ($meta_value) {
                ...
                $contents[] = ['source_id' => $source_id, 'name' => $post_id, 'content' => ExtractionCache::remember('bricks', $source_id, ['meta' => $meta_value, 'global_classes' => $this->global_classes_index, 'version' => \BRICKS_VERSION], function () use ($meta_value): array {
            ...
            if (array_key_exists('settings', $node)) {
                ...
                if (array_key_exists('_cssGlobalClasses', $node['settings'])) {
                    ...
                    if (is_array($global_classes)) {
                        $meta_value[$key]['settings']['_cssGlobalClasses'] = array_map(fn($class) => array_key_exists($class, $this->global_classes_index) ? $this->global_classes_index[$class] : $class, $global_classes);
                    }
                }
    ...
    public function get_components(): array
    {
        ...
        return ExtractionCache::remember('bricks', 'option:bricks_components', ['components' => $components, 'global_classes' => $this->global_classes_index, 'version' => \BRICKS_VERSION], function () use ($components): array {
