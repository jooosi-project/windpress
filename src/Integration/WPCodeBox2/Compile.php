...
class Compile
{
    ...
    public function get_contents($metadata): array
    {
        ...
        if (is_string($cursor) && strpos($cursor, 'snippets:') === 0) {
            $state = json_decode((string) base64_decode(substr($cursor, 9), \true), \true);
            if (!is_array($state) || !isset($state['after'], $state['max'], $state['size']) || !is_int($state['after']) || !is_int($state['max']) || !is_int($state['size']) || $state['after'] < 0 || $state['max'] < $state['after'] || $state['size'] < 1 || $state['size'] > 200) {
        ...
        foreach ($results as $result) {
            $contents[] = ['source_id' => 'snippet:' . $result['id'], 'id' => $result['id'], 'title' => sprintf('#%s: %s', $result['id'], $result['title']), 'content' => $result['original_code']];
        }
        ...
        return ['metadata' => ['total_batches' => \false, 'next_batch' => $has_more ? 'snippets:' . base64_encode(wp_json_encode(['after' => $last_id, 'max' => $maximum_id, 'size' => $per_page])) : \false], 'contents' => $contents];
    }
}
