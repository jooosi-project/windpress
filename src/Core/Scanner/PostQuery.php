...
class PostQuery
{
    ...
    public function __construct(array $arguments, array $metadata = [])
    {
        ...
        if (is_string($cursor) && strpos($cursor, 'posts:') === 0) {
            $state = json_decode((string) base64_decode(substr($cursor, 6), true), true);
            if (! is_array($state)
                || ($state['query'] ?? null) !== $this->signature
                || ! isset($state['after'], $state['max'], $state['size'])
        ...
        try {
            ...
            if ((int) $this->query->query_vars['posts_per_page'] !== $this->batch_size
                || (int) ($this->query->query_vars['offset'] ?? 0) !== 0
                || count($this->query->posts) > $this->batch_size
            ) {
    ...
    public function metadata(): array
    {
        ...
        return [
            'next_batch' => $has_more ? 'posts:' . base64_encode(wp_json_encode([
                'after' => $last_id,
                'max' => $this->maximum_id,
