...
class SourceIndex
{
    ...
    public static function scan(string $scope, array $metadata, callable $fetch): array
    {
        ...
        if (! is_array($request) || ($request['version'] ?? null) !== 1
            || ! array_key_exists('baseline', $request)
            || ($request['baseline'] !== null && (! is_string($request['baseline']) || ! preg_match('/^[a-f0-9]{64}$/D', $request['baseline'])))
            || ! in_array($metadata['kind'] ?? 'incremental', ['full', 'incremental'], true)) {
            throw new InvalidArgumentException(__('The source index request is invalid.', 'windpress'));
        }

        $scope = hash('sha256', serialize([
            1, WIND_PRESS::VERSION, get_current_blog_id(), get_current_user_id(), get_locale(), $scope,
            apply_filters('f!windpress/core/scanner/source_index:scope', '', $scope),
        ]));
        ...
        try {
            ...
            foreach ($result['contents'] as $source) {
                if (! is_array($source) || ! is_string($source['source_id'] ?? null) || $source['source_id'] === ''
                    || strlen($source['source_id']) > 2048 || ! is_string($source['content'] ?? null)
                    || (isset($source['type']) && ! is_string($source['type']))) {
            ...
            $result['metadata']['source_index'] = [
                'version' => 1,
                'mode' => $state['baseline'] === null ? 'snapshot' : 'delta',
                'scope' => $scope,
                'base_revision' => $state['baseline'],
                'revision' => $revision,
                'deleted' => $deleted,
                'examined' => $examined,
                'changed' => count($contents),
                'unchanged' => $examined - count($contents),
            ];
            if (! ScanLock::is_owner($key, $lease)) {
    ...
    private static function save_manifest(string $scope, string $revision, array $sources): void
    {
        ...
        $lease = ScanLock::acquire($registry_key);
        if ($lease === null) {
        ...
        try {
            ...
            ScanLock::release($registry_key, $lease);
        }
    }
