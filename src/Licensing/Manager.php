...
final class Manager
{
    ...
    {
        return ['id' => self::INTEGRATION_ID, 'url' => WIND_PRESS::EDD_STORE['store_url'], 'item_id' => WIND_PRESS::EDD_STORE['item_id'], 'version' => WIND_PRESS::VERSION, 'file' => WIND_PRESS::FILE, 'option_name' => self::OPTION_NAME, 'weekly_check' => \true];
    }
    /**
    ...
    public function boot_updater(): void
    {
        ...
        new PluginUpdater(WIND_PRESS::EDD_STORE['store_url'], ['file' => WIND_PRESS::FILE, 'item_id' => WIND_PRESS::EDD_STORE['item_id'], 'version' => WIND_PRESS::VERSION, 'license' => $license_key, 'beta' => $this->is_pre_release_enabled(), 'allow_tracking' => $this->license->get_allow_tracking(), 'wp_override' => \true], new Messenger());
    }
    ...
    public function activate(string $license_key)
    {
        ...
        if ($license_key === '') {
            return new WP_Error('missing', __('Enter a license key.', 'windpress'));
        }
    ...
    private function request(string $action, string $license_key)
    {
        ...
        $license_data = $api->make_request(['edd_action' => $action, 'license' => $license_key, 'item_id' => WIND_PRESS::EDD_STORE['item_id']]);
        if (!is_object($license_data)) {
            return new WP_Error('license_server_unavailable', __('The license server could not be reached.', 'windpress'));
        }
