...
class Cache extends AbstractApi implements ApiInterface
{
    ...
    public function index(WP_REST_Request $wprestRequest): WP_REST_Response
    {
        ...
        $cache = [
            ...
            'file_url' => null,
            'file_size' => false,
            'generation' => $build_state['generation'],
    ...
    public function store(WP_REST_Request $wprestRequest): WP_REST_Response
    {
        ...
        if (! is_array($payload) || ! isset($payload['content']) || ! is_string($payload['content'])
            || (isset($payload['expected_generation']) && ! is_string($payload['expected_generation']))
            || (isset($payload['expected_source_revision']) && ! is_string($payload['expected_source_revision']))
            || (isset($payload['sourcemap']) && ! is_string($payload['sourcemap']))
            || (isset($payload['full_build']) && ! is_bool($payload['full_build']) && ! is_int($payload['full_build']))
        ) {
            return new WP_REST_Response([
                'status' => 'KO',
                'message' => __('The cache payload is invalid.', 'windpress'),
            ], 400);
        }

        $content = base64_decode($payload['content'], true);
        $sourcemapContent = ! empty($payload['sourcemap']) ? base64_decode($payload['sourcemap'], true) : null;
        if ($content === false || $sourcemapContent === false) {
            return new WP_REST_Response([
                'status' => 'KO',
                'message' => __('The cache content is not valid base64.', 'windpress'),
            ], 400);
        }
        ...
        if ($lease === null) {
            return new WP_REST_Response([
                'status' => 'KO',
                'message' => __('Another cache build is being saved. Retry the build.', 'windpress'),
            ], 409);
        }
        ...
        try {
            if (isset($payload['expected_generation']) && ! BuildState::matches($payload['expected_generation'])) {
                return new WP_REST_Response([
                    'status' => 'KO',
                    'message' => __('The scan generation has changed. Restart the build.', 'windpress'),
                ], 409);
            }
            ...
            if (isset($payload['expected_source_revision']) && ! SourceRevision::matches($payload['expected_source_revision'])) {
                return new WP_REST_Response([
                    'status' => 'KO',
                    'message' => __('Source content changed during the scan. Restart the build.', 'windpress'),
                ], 409);
            }
    ...
    public function providers_scan(WP_REST_Request $wprestRequest): WP_REST_Response
    {
        ...
        if (! is_string($provider_id) || ! is_array($metadata)
            || (isset($metadata['generation']) && ! is_string($metadata['generation']))
            || (isset($metadata['source_revision']) && ! is_string($metadata['source_revision']))
        ) {
            return new WP_REST_Response([
                'status' => 'KO',
                'message' => __('The scan request is invalid.', 'windpress'),
            ], 400);
        }
        ...
        try {
            ...
            return new WP_REST_Response([
                'status' => 'KO',
                'message' => __('Provider error: ', 'windpress') . $throwable->getMessage(),
            ], $throwable instanceof \InvalidArgumentException ? 400 : ($throwable->getCode() === 409 ? 409 : 500));
        } finally {
